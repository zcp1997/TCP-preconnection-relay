...
static void log_enqueue_v(const char *fmt, va_list ap) {
...
static void log_flush_rate_limited(uint64_t now_ms) {
    if (!LOG_ENABLE) return;
...
static bool pool_put_locked(int fd, uint64_t now_ms) {
...
static int pool_get_locked(void) {
    if (pool_count > 0) return pool[--pool_count].fd;
    return -1;
}

static void *thread_refill(void *arg) {
    (void)arg;

    // 安全获取最新的远端地址
    struct sockaddr_storage tmp_addr;
    socklen_t tmp_len;
    pthread_rwlock_rdlock(&addr_rwlock);
    tmp_addr = remote_tcp_addr;
    tmp_len = remote_tcp_addrlen;
    pthread_rwlock_unlock(&addr_rwlock);

    int s = socket(((struct sockaddr*)&tmp_addr)->sa_family, SOCK_STREAM, 0);
    if (s < 0) goto fin;

    set_tcp_socket_options(s);
    if (set_nonblock(s) != 0) { close(s); goto fin; }

    int rc = connect(s, (struct sockaddr*)&tmp_addr, tmp_len);
...
static void *thread_maintain(void *arg) {
    (void)arg;
    while (1) {
        uint64_t now = mono_ms();
...
// DDNS 动态刷新后台线程
static void *thread_dns_refresh(void *arg) {
    (void)arg;
    char target_domain[256];
    strncpy(target_domain, REMOTE_IP, sizeof(target_domain) - 1);
    target_domain[sizeof(target_domain)-1] = '\0';

    while (1) {
        sleep(60); // 每 60 秒解析一次域名

        struct sockaddr_storage new_tcp_addr, new_udp_addr;
        socklen_t new_tcp_len, new_udp_len;
        
        // 尝试重新解析 TCP 域名
        if (resolve_addr(target_domain, REMOTE_TCP_PORT, SOCK_STREAM, &new_tcp_addr, &new_tcp_len) == 0) {
            pthread_rwlock_rdlock(&addr_rwlock);
            int changed = memcmp(&new_tcp_addr, &remote_tcp_addr, new_tcp_len);
            pthread_rwlock_unlock(&addr_rwlock);

            if (changed != 0) {
                pthread_rwlock_wrlock(&addr_rwlock);
                remote_tcp_addr = new_tcp_addr;
                remote_tcp_addrlen = new_tcp_len;
                pthread_rwlock_unlock(&addr_rwlock);
                
                log_enqueue("DDNS Updated: Target TCP IP has smoothly changed to the new address.");
            }
        }

        // 尝试重新解析 UDP 域名
        if (resolve_addr(target_domain, REMOTE_UDP_PORT, SOCK_DGRAM, &new_udp_addr, &new_udp_len) == 0) {
            pthread_rwlock_rdlock(&addr_rwlock);
            int changed = memcmp(&new_udp_addr, &remote_udp_addr, new_udp_len);
            pthread_rwlock_unlock(&addr_rwlock);

            if (changed != 0) {
                pthread_rwlock_wrlock(&addr_rwlock);
                remote_udp_addr = new_udp_addr;
                remote_udp_addrlen = new_udp_len;
                pthread_rwlock_unlock(&addr_rwlock);
                
                log_enqueue("DDNS Updated: Target UDP IP has smoothly changed to the new address.");
            }
        }
    }
    return NULL;
...
static UdpAssoc* udp_get_or_create(const struct sockaddr_storage *cli,
    ...
    // 安全获取最新 UDP 地址
    struct sockaddr_storage tmp_udp_addr;
    socklen_t tmp_udp_len;
    pthread_rwlock_rdlock(&addr_rwlock);
    tmp_udp_addr = remote_udp_addr;
    tmp_udp_len = remote_udp_addrlen;
    pthread_rwlock_unlock(&addr_rwlock);

    int fd = socket(((struct sockaddr*)&tmp_udp_addr)->sa_family, SOCK_DGRAM, 0);
    if (fd < 0) return NULL;
    set_udp_socket_options(fd);
    if (set_nonblock(fd) != 0) { close(fd); return NULL; }
    if (connect(fd, (struct sockaddr*)&tmp_udp_addr, tmp_udp_len) != 0) { close(fd); return NULL; }
...
int main() {
    ...
    // 启动 DDNS 刷新线程
    pthread_t t_dns;
    pthread_create(&t_dns, NULL, thread_dns_refresh, NULL);
    ...
    while (1) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (tagp == NULL) {
                while (1) {
                    ...
                    if (rem < 0) {
                        ...
                        // 安全获取最新 TCP 地址用于直连 fallback
                        struct sockaddr_storage tmp_tcp_addr;
                        socklen_t tmp_tcp_len;
                        pthread_rwlock_rdlock(&addr_rwlock);
                        tmp_tcp_addr = remote_tcp_addr;
                        tmp_tcp_len = remote_tcp_addrlen;
                        pthread_rwlock_unlock(&addr_rwlock);

                        rem = socket(((struct sockaddr*)&tmp_tcp_addr)->sa_family, SOCK_STREAM, 0);
                        if (rem < 0) { close(cli); continue; }
                        set_tcp_socket_options(rem);
                        if (set_nonblock(rem) != 0) { close(rem); close(cli); continue; }
                        
                        int rc = connect(rem, (struct sockaddr*)&tmp_tcp_addr, tmp_tcp_len);
                        if (rc != 0) {
            ...
            if (is_remote_fd && c->connecting) {
                if (events[i].events & (EPOLLOUT | EPOLLERR | EPOLLHUP | EPOLLRDHUP)) {
                    int err = 0;
                    socklen_t len = sizeof(err);
